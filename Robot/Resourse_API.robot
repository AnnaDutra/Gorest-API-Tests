...
Editar email do usuario para "${Email_Novo}"
    
    ${RESPOSTA_PUT}    PUT On Session    gorest    users/${RESPOSTA_POST.json()["id"]}
    ...                data={"name": "${usuario_1.name}","email":"${Email_Novo}","gender":"${usuario_1.gender}","status":"${usuario_1.status}" }
    ...                headers=${HEADERS}
    Log                ${RESPOSTA_PUT.text} 
    Should Be Equal    ${RESPOSTA_PUT.json()["email"]}    ${Email_Novo}
